...
for tol in [1e-3, 1e-4]
    ...
    println(
        "status: $(output.status), total_iters: $(output.total_iters), kkt_error: $(fmt(output.kkt_error))",
    )
    analyze("  converged point", collect(output.x[1:n_primal]))
    JLD2.save_object(
        joinpath(@__DIR__, "results", "tight_tol_3level_tol$(tol).jld2"),
        Dict(
            "x" => collect(output.x),
