...
for run_id in run_ids
	...
	sol = JLD2.load_object(joinpath(dir, "solution", "solution_dict_instance_1_eps0.1.jld2"))
	...
	nviol(r) = count(<(0), r)
	println("violations: safety $(nviol(safety))/$T (min $(fmt(minimum(safety)))), ",
	...
	println("player 1: control = $(fmt(control_cost)), goal = $(fmt(goal_val)), load_balance = $(fmt(load_balance)), robot_ineq penalty (level $robot_level) = $(fmt(penalty(robot_h, robot_level)))")
	println("player 2: pot_approach = $(fmt(pot_approach)), child_speed penalty (level 2) = $(fmt(penalty(child_speed, 2)))")
