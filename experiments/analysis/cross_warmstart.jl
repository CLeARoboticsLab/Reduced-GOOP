...
prob4 = JLD2.load_object(
    joinpath(
        runs_root,
        "Robotic_arm_single_robot_agent_4_levels",
        "data",
        "problem",
        "problem_data_instance_1.jld2",
    ),
)
prob4 isa AbstractVector && (prob4 = prob4[1])
sol3 = JLD2.load_object(
    joinpath(
        runs_root,
...
println(
    "status: $(output.status), total_iters: $(output.total_iters), kkt_error: $(round(output.kkt_error; sigdigits = 5))",
)

x = collect(output.x[1:n_primal])
mkpath(joinpath(@__DIR__, "results"))
JLD2.save_object(
    joinpath(@__DIR__, "results", "cross_warmstart_solution.jld2"),
    Dict(
        "x" => x,
        "z" => collect(output.z),
...
function analyze(label, primal)
    strategies = Robotic_arm.extract_player_strategies(
        primal,
        problem.primal_dims,
        scenario_config.dynamics,
    )
    ...
    goal_val = sum(
        abs2,
        0.5 .* (arm.xs[end][1:3] .+ arm.xs[end][4:6]) .- 0.5 .* (goal1 .+ goal2),
    )
    control = sum(sum(u .^ 2) for u in arm.us)
    speed_res = vcat(
        [25.0 - sum(abs2, u[1:3]) for u in arm.us[1:(end - 1)]],
        [25.0 - sum(abs2, u[4:6]) for u in arm.us[1:(end - 1)]],
    )
    safety = minimum(
        sum(abs2, 0.5 .* (arm.xs[t][1:3] .+ arm.xs[t][4:6]) .- child.xs[t]) - 4.0 for
        t in 1:T
    )
    pot = sum(
        sum(abs2, child.xs[t] .- 0.5 .* (arm.xs[t][1:3] .+ arm.xs[t][4:6])) for t in 1:T
    )
    println(
        "$label: goal = $(fmt(goal_val)), control = $(fmt(control)), worst speed residual = $(fmt(minimum(speed_res))), Σ overshoot = $(fmt(-sum(min.(speed_res, 0.0)))), safety margin = $(fmt(safety)), pot_approach = $(fmt(pot))",
    )
...
println(
    "‖converged − warmstart‖        = ",
    fmt(sqrt(sum(abs2, r_new.primal .- r_ws.primal))),
)
println(
    "‖converged − default 3-level‖  = ",
    fmt(sqrt(sum(abs2, r_new.primal .- r_def.primal))),
)
println(
    "‖warmstart − default 3-level‖  = ",
    fmt(sqrt(sum(abs2, r_ws.primal .- r_def.primal))),
)
