...
run_dir = joinpath(REPO_ROOT, "data", "robotic_arm_open_loop", "runs", run_id, "data", "problem")
meta = JLD2.load_object(joinpath(run_dir, "run_metadata.jld2"))
sol = JLD2.load_object(joinpath(run_dir, "solution", "solution_dict_instance_1_eps0.1.jld2"))
...
println("run: $run_id")
println("limits: arm_speed=$(arm_speed_limit), child_speed=$(child_speed_limit), collision_avoidance=$(collision_avoidance), dₚ=$(d_p)")
println("status: ", sol["status"], "   kkt_error: ", sol["kkt_error"], "   ϵ: ", sol["ϵ"])
haskey(sol, "preference_values") && println("stored preference_values: ", sol["preference_values"])
...
println("\n── Summary ──")
println("player 1 worst residuals: safety = $(fmt(minimum(safety))), arm1 speed = $(fmt(minimum(speed1))), arm2 speed = $(fmt(minimum(speed2)))")
println("  combined robot_inequality min = $(fmt(min(minimum(safety), minimum(speed1), minimum(speed2))))")
