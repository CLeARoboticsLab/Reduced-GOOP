...
preferences4 =
    [vcat([control_objective_p1], problem.preferences[1]), problem.preferences[2]]
is_constraint4 = [
...
JLD2.save_object(
    joinpath(@__DIR__, "results", "cross_warmstart_reverse_solution.jld2"),
    Dict(
        "x" => x,
        "z" => collect(output.z),
...
function analyze(label, primal)
    ...
    println(
        "$label: goal = $(fmt(goal_val)), control = $(fmt(control)), safety margin = $(fmt(safety))",
    )
