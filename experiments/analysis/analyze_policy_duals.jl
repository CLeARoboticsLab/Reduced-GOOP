...
ψ_indices = sort(reduce(vcat, [idx for (name, idx) in groups if startswith(name, "ψ")]; init = Int[]))
println("total ψ coordinates: $(length(ψ_indices)) of $(kkt.variable_dimension)")

residual(z) = (val = zeros(kkt.kkt_dimension); kkt.F!(val, z; θ = collect(θ), ϵ = 0.1, η = 0.0); val)

runs_root = joinpath(REPO_ROOT, "data", "robotic_arm_open_loop", "runs")
points = [
	"swerving 3-level equilibrium" =>
		collect(JLD2.load_object(joinpath(runs_root, "Robotic_arm_single_robot_agent_3_levels", "data", "problem", "solution", "solution_dict_instance_1_eps0.1.jld2"))["z"]),
	"direct-basin 3-level point  " =>
		collect(JLD2.load_object(joinpath(@__DIR__, "results", "cross_warmstart_solution.jld2"))["z"]),
]
...
for (label, z) in points
	...
	F = residual(z)
	zψ0 = copy(z); zψ0[ψ_indices] .= 0.0
	F0 = residual(zψ0)
	ΔF = F0 .- F
	println("\n══ $label ══")
	println("  ‖F(z)‖₂ = $(fmt(norm(F)))  (solver kkt_error metric),  ‖F(z)‖∞ = $(fmt(norm(F, Inf)))")
	for name in sort(collect(keys(groups)))
		startswith(name, "ψ") || continue
		v = z[groups[name]]
		println("  $name: max|ψ| = $(fmt(maximum(abs, v))), ‖ψ‖₂ = $(fmt(norm(v))), mean|ψ| = $(fmt(sum(abs, v) / length(v)))")
	end
	for name in sort(collect(keys(groups)))
	...
	println("  ψ contribution to residual rows: max|ΔF| = $(fmt(norm(ΔF, Inf))), #rows with |ΔF| > 0.01: $(count(>(0.01), abs.(ΔF)))")
end
