...
function run_goop_experiments(; num_iters::Int = 100, seed::Int = 0)
    ...
    g_eq(x, θ) = [x[1]^3 + x[2] + x[3]^2 + x[4]^5 - 1.0;
                  cos(x[3])]
    ...
    for iteration in 1:num_iters
        ...
        goop_prefs = [[J₁, J₂, J₃]]  # single player
        ...
        prob = isnothing(inequality_constraints[1]) ?
            NonlinearProblem(test_f!, z0) :
            NonlinearLeastSquaresProblem(test_f!, z0)
