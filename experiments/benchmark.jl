...
using LinearAlgebra: I, Diagonal, norm, pinv, qr, rank, svd, nullspace
using Symbolics
...
"""
Construct a parametric GOOP benchmark instance for either `:QP` or `:NQP`.

Each player/preference uses either a quadratic objective (`:QP`) or a quadratic
objective with an added quartic term (`:NQP`).

Constraint structure is chosen so that `z = 0` is initially feasible:
- Equality constraints: `H * z - 0 = 0` (implemented as `H * z`).
- Inequality constraints: `1 - G * z >= 0`.
"""
function get_setup(
	n,
	num_players,
	...
	problem_type = parse_problem_type(problem_type)
	...
	# equality_constraints[1] = (z, θ) -> [z[2] - z[6]]
	# equality_constraints[2] = (z, θ) -> [z[5] - z[3]]
	...
	# inequality_constraints[1] = (z, θ) -> [z[1] - 4.0]
	# inequality_constraints[2] = (z, θ) -> [z[4] - 4.0]
...
"""
Run the reduced-vs-complete KKT benchmark for either `:QP` or `:NQP`.

This generates random problem instances, solves both systems, discards the first
solved instance as warmstart, and saves per-instance plus aggregate metrics,
histories, and convergence plots under a mode-tagged `run_id`.
Note: Qk matrices are psd with rank n/2
...
function demo(;
	...
	problem_type = parse_problem_type(problem_type)
	problem_type_tag = String(problem_type)

	# GOOP Problem setup
	n = 10 # x primal dimension (per player), nⁱ
	...
	local_order_histories_reduced = Vector{Vector{Float64}}()
	local_order_histories_complete = Vector{Vector{Float64}}()

	# Create problem
	problem = get_setup(n, num_players, mₑ, mᵢ; num_preferences, param_distribution, problem_type)
	...
	while solved_attempts < num_instances + 1
		...
		if solved_attempts > 0
			...
			z = reduced_z
			@save joinpath(solution_data_dir, "$(problem_type_tag)_reduced_solution_instance_$(instance_idx).jld2") solved_instance_idx primal z reduced_pref_values
			primal = complete_primal
			z = complete_z
			@save joinpath(solution_data_dir, "$(problem_type_tag)_complete_solution_instance_$(instance_idx).jld2") solved_instance_idx primal z complete_pref_values

			# Save problem parameters
			@save joinpath(solution_data_dir, "$(problem_type_tag)_problem_parameters_instance_$(instance_idx).jld2") flattened_parameters
			...
			reduced_p_iterations, reduced_p_history = local_order_sequence_from_log(reduced_kkt_history)
			complete_p_iterations, complete_p_history = local_order_sequence_from_log(complete_kkt_history)
			push!(local_order_histories_reduced, reduced_p_history)
			push!(local_order_histories_complete, complete_p_history)
			kkt_error_history = reduced_kkt_history
			@save joinpath(histories_data_dir, "$(problem_type_tag)_kkt_error_history_reduced_instance_$(instance_idx).jld2") kkt_error_history
			kkt_error_history = complete_kkt_history
			@save joinpath(histories_data_dir, "$(problem_type_tag)_kkt_error_history_complete_instance_$(instance_idx).jld2") kkt_error_history
			save_local_order_instance_table(
				joinpath(histories_data_dir, "$(problem_type_tag)_local_order_instance_$(instance_idx).csv");
				reduced_iterations = reduced_p_iterations,
				reduced_values = reduced_p_history,
				complete_iterations = complete_p_iterations,
				complete_values = complete_p_history,
			)
		else
	...
	@save joinpath(histories_data_dir, "$(problem_type_tag)_pref_l2_diffs_per_player.jld2") pref_l2_diffs_per_player
	@save joinpath(histories_data_dir, "$(problem_type_tag)_kkt_error_histories_reduced.jld2") kkt_error_histories_reduced
	@save joinpath(histories_data_dir, "$(problem_type_tag)_kkt_error_histories_complete.jld2") kkt_error_histories_complete
	@save joinpath(histories_data_dir, "$(problem_type_tag)_local_order_histories_reduced.jld2") local_order_histories_reduced
	@save joinpath(histories_data_dir, "$(problem_type_tag)_local_order_histories_complete.jld2") local_order_histories_complete
	absolute_differences = abs.(reduced_elapsed_times .- complete_elapsed_times)
	@save joinpath(histories_data_dir, "$(problem_type_tag)_elapsed_times.jld2") reduced_elapsed_times complete_elapsed_times absolute_differences
	...
	if !isempty(kkt_error_histories_reduced) && any(!isempty, kkt_error_histories_reduced)
		...
		CairoMakie.save(
			joinpath(convergence_plots_dir, "convergence_aggregate_reduced_$(problem_type_tag).pdf"),
	...
	if !isempty(kkt_error_histories_complete) && any(!isempty, kkt_error_histories_complete)
		...
		CairoMakie.save(
			joinpath(convergence_plots_dir, "convergence_aggregate_complete_$(problem_type_tag).pdf"),
	...
	if (!isempty(kkt_error_histories_reduced) && any(!isempty, kkt_error_histories_reduced)) &&
	   (!isempty(kkt_error_histories_complete) && any(!isempty, kkt_error_histories_complete))
		...
		CairoMakie.save(
			joinpath(convergence_plots_dir, "convergence_aggregate_reduced_vs_complete_$(problem_type_tag)_$(num_preferences)prefs_$(ϵ₀)rho.pdf"),
			combined_aggregate_convergence_fig,
		)
	end

	if (!isempty(kkt_error_histories_reduced) && any(length(history) >= 3 for history in kkt_error_histories_reduced)) &&
	   (!isempty(kkt_error_histories_complete) && any(length(history) >= 3 for history in kkt_error_histories_complete))
		local_order_fig, _, reduced_local_order_stats, complete_local_order_stats = plot_local_order_aggregate_comparison(
			;
			reduced_log_kkt_error_histories = kkt_error_histories_reduced,
			complete_log_kkt_error_histories = kkt_error_histories_complete,
			show_legend = show_convergence_legend,
			show_ylabel = show_ylabel,
		)
		CairoMakie.save(
			joinpath(convergence_plots_dir, "local_order_aggregate_reduced_vs_complete_$(problem_type_tag)_$(num_preferences)prefs_$(ϵ₀)rho.pdf"),
			local_order_fig,
		)
		save_local_order_aggregate_table(
			joinpath(histories_data_dir, "$(problem_type_tag)_local_order_aggregate.csv");
			reduced_stats = reduced_local_order_stats,
			complete_stats = complete_local_order_stats,
		)
		isfinite(reduced_local_order_stats.p_hat) && println("[LocalOrder] reduced p_hat (tail mean): $(round(reduced_local_order_stats.p_hat, digits = 6))")
		isfinite(complete_local_order_stats.p_hat) && println("[LocalOrder] complete p_hat (tail mean): $(round(complete_local_order_stats.p_hat, digits = 6))")
	end
...
# Helper functions 
const SUPPORTED_PROBLEM_TYPES = (:QP, :NQP)
const SUPPORTED_PARAM_DISTRIBUTIONS = (:normal, :uniform, :laplace, :student_t5, :rademacher)

"""
Normalize and validate the benchmark `problem_type` symbol.

Accepted values are `:QP` and `:NQP` (case-insensitive string/symbol input).
"""
function parse_problem_type(problem_type)
	mode = Symbol(uppercase(String(problem_type)))
	if mode ∉ SUPPORTED_PROBLEM_TYPES
		throw(
			ArgumentError(
				"Unsupported `problem_type=$(problem_type)`. Choose one of $(SUPPORTED_PROBLEM_TYPES).",
			),
		)
	end
	return mode
...
"""
Normalize and validate the random-parameter sampling distribution selector.
"""
...
"""
Return a unit-variance distribution object for the requested parameter family.
"""
...
"""
Sample a positive semidefinite `n x n` matrix with rank at most `r`.
"""
...
"""
Return an orthonormal basis for `Col(A)` using an SVD threshold.
"""
function colspace_basis(A::AbstractMatrix; tol::Real = 1e-10)
	F = svd(A)
	r = count(>(tol), F.S)
	r == 0 && return zeros(eltype(A), size(A, 1), 0)
	F.U[:, 1:r]
end

"""
Return a basis for the intersection of column spaces of all matrices in `matrices`.
"""
function colspace_intersection_basis(matrices::AbstractVector{<:AbstractMatrix}; tol::Real = 1e-10)
	isempty(matrices) && throw(ArgumentError("matrices must be non-empty"))
	n_rows = size(first(matrices), 1)
	any(size(M, 1) != n_rows for M in matrices) &&
		throw(ArgumentError("all matrices must have the same number of rows"))

	basis = colspace_basis(first(matrices); tol)
	for M in Iterators.drop(matrices, 1)
		next_basis = colspace_basis(M; tol)
		if size(basis, 2) == 0 || size(next_basis, 2) == 0
			return zeros(eltype(first(matrices)), n_rows, 0)
		end

		# Solve basis*α = next_basis*β via nullspace([basis -next_basis]).
		coupling_null = nullspace([basis -next_basis]; atol = tol, rtol = 0.0)
		if size(coupling_null, 2) == 0
			return zeros(eltype(first(matrices)), n_rows, 0)
		end

		α = coupling_null[1:size(basis, 2), :]
		basis = colspace_basis(basis * α; tol)
	end
	basis
end
...
"""
Print per-player preference values evaluated at the provided primal solution.
"""
...
"""
Print per-player constraint residuals evaluated at the provided primal solution.

Residuals are reported as `H*z` and `1 - G*z` when generated by this benchmark's parameterization).
"""
function print_constraint_residuals(label, num_players, n, z, θ; mₑ, mᵢ, num_preferences)
	...
	for player_idx in 1:num_players
		...
		eq_residual = H * primal
		ineq_residual = 1 .- G * primal
		println("  player $(player_idx): H*z = $(round.(eq_residual, digits = 6))")
		println("  player $(player_idx): 1 - G*z  = $(round.(ineq_residual, digits = 6))")
	end
end

"""
Evaluate all players' preference functions at the primal block of `z`.
"""
...
"""
Save a per-instance CSV table for local order values `p_k`.
"""
function save_local_order_instance_table(path; reduced_iterations, reduced_values, complete_iterations, complete_values)
	reduced_lookup = Dict(k => i for (i, k) in enumerate(reduced_iterations))
	complete_lookup = Dict(k => i for (i, k) in enumerate(complete_iterations))
	max_k = max(
		isempty(reduced_iterations) ? 0 : maximum(reduced_iterations),
		isempty(complete_iterations) ? 0 : maximum(complete_iterations),
	)

	open(path, "w") do io
		println(io, "k,reduced_p,complete_p")
		for k in 1:max_k
			reduced_idx = get(reduced_lookup, k, 0)
			complete_idx = get(complete_lookup, k, 0)
			reduced_val = reduced_idx == 0 ? "" : string(reduced_values[reduced_idx])
			complete_val = complete_idx == 0 ? "" : string(complete_values[complete_idx])
			println(io, "$(k),$(reduced_val),$(complete_val)")
		end
	end
	return nothing
end
...
"""
Save an aggregate CSV table for local convergence order statistics.

The file stores per-iteration mean/std/count for reduced and complete systems,
plus tail-mean estimates (`p_hat`) in comment lines.
"""
function save_local_order_aggregate_table(path; reduced_stats, complete_stats)
	reduced_lookup = Dict(k => i for (i, k) in enumerate(reduced_stats.x))
	complete_lookup = Dict(k => i for (i, k) in enumerate(complete_stats.x))
	max_k = max(
	...
	open(path, "w") do io
		println(io, "k,reduced_mean,reduced_std,reduced_n,complete_mean,complete_std,complete_n")
		for k in 1:max_k
			reduced_idx = get(reduced_lookup, k, 0)
			complete_idx = get(complete_lookup, k, 0)
			reduced_mean = reduced_idx == 0 ? "" : string(reduced_stats.y_mean[reduced_idx])
			reduced_std = reduced_idx == 0 ? "" : string(reduced_stats.y_std[reduced_idx])
			reduced_n = reduced_idx == 0 ? "" : string(reduced_stats.counts[reduced_idx])
			complete_mean = complete_idx == 0 ? "" : string(complete_stats.y_mean[complete_idx])
			complete_std = complete_idx == 0 ? "" : string(complete_stats.y_std[complete_idx])
			complete_n = complete_idx == 0 ? "" : string(complete_stats.counts[complete_idx])
			println(io, "$(k),$(reduced_mean),$(reduced_std),$(reduced_n),$(complete_mean),$(complete_std),$(complete_n)")
		end
		println(io, "# reduced_p_hat,$(reduced_stats.p_hat)")
		println(io, "# complete_p_hat,$(complete_stats.p_hat)")
	end
	return nothing
end

"""
Generate a random benchmark parameter object.

The generated constraints satisfy:
- `h = H * z_feas` with `z_feas = 0`, so equality constraints are feasible.
- `g = 1 - G * z_feas`, so inequality constraints `1 - G*z >= 0` are strictly
  feasible at `z = 0`.

Supported `param_distribution`: `:normal`, `:uniform`, `:laplace`,
`:student_t5`, `:rademacher`.
"""
function generate_random_parameter(;
	...
	for i in 1:num_players
		...
		H = hcat(H_blocks...)
		# For player i, use diagonal Q blocks and enforce Gᵢ' in their column space intersection across all preference levels.
		player_block = ((i - 1) * n + 1):(i * n)
		aggregate_Q_full = [preference_data[k].Qk[player_block, player_block] for k in 1:num_preferences]
		G_blocks = [rand(distribution, mᵢ, n) * 0.1 for _ in 1:num_players]
		# intersection_basis = colspace_intersection_basis(aggregate_Q_full)
		# if size(intersection_basis, 2) == 0
		# 	@error "Intersection of column spaces for player $(i) is empty"
		# else
		# 	Main.@infiltrate
		# 	G_blocks[i] = (intersection_basis * (rand(distribution, size(intersection_basis, 2), mᵢ) * 0.1))'
		# end
		
		G_blocks[i] = (preference_data[end].Qk[player_block, player_block] * (rand(distribution, n, mᵢ) * 0.1))'
		G = hcat(G_blocks...)


		z_feas = zeros(num_players * n) # keep zero vector as feasible point
		h = H * z_feas # feasible equality constraints # can be dropped actually
		g = ones(mᵢ) - G * z_feas # feasible inequality constraints
