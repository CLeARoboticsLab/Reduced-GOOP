...
function _single_integrator_3d_plot_data(;
    ...
    x3, y3, z3 =
        has_child ? _trajectory_xyz(strategy[3].xs; player = 3) :
        (Float64[], Float64[], Float64[])

    start1 =
        isnothing(initial_position1) ? _initial_position_from_parameter_block(θ1, "θ1") :
        _position3(initial_position1, "initial_position1")
    start2 =
        isnothing(initial_position2) ? _initial_position_from_parameter_block(θ2, "θ2") :
    ...
    xlims =
        isnothing(map_end) ? _padded_limits(all_x; margin = workspace_margin) :
        _padded_limits(
            all_x;
            lower = -map_end,
            upper = map_end,
    ...
    ylims =
        isnothing(map_end) ? _padded_limits(all_y; margin = workspace_margin) :
        _padded_limits(
            all_y;
            lower = -map_end,
            upper = map_end,
...
function draw_single_integrator_3d_environment!(
    ...
    for z in (floor_z, zlims[2])
        for idx in eachindex(corners)
            ...
            makie.lines!(ax, [x1, x2], [y1, y2], [z, z]; color = box_color, linewidth = 1)
        end
    end
    for (x, y) in corners
        makie.lines!(
...
function _plot_single_integrator_3d_trajectories(
    ...
    draw_single_integrator_3d_environment!(makie, ax; xlims, ylims, zlims, grid_spacing)
    ...
    if has_child && !isnothing(collision_avoidance)
        ...
        for t in safety_indices
            hemisphere_segments =
                _hemisphere_wireframe([x3[t], y3[t], z3[t]], collision_avoidance)
...
function _plot_single_integrator_3d_browser_trajectories(
    ...
    player1_path =
        makie.lines!(scene_axis, x1, y1, z1; color = player1_color, linewidth = 4)
    player2_path =
        makie.lines!(scene_axis, x2, y2, z2; color = player2_color, linewidth = 4)
    player3_path = nothing
    if has_child
        player3_path = makie.lines!(
            scene_axis,
    ...
    current1_position =
        makie.lift(t -> _agent_position_at_time(makie, t, x1, y1, z1), time_index)
    current2_position =
        makie.lift(t -> _agent_position_at_time(makie, t, x2, y2, z2), time_index)
    ...
    current3_marker = if has_child
        current3_position =
            makie.lift(t -> _agent_position_at_time(makie, t, x3, y3, z3), time_index)
        makie.scatter!(
            scene_axis,
            current3_position;
            marker = :diamond,
    ...
    append!(
        legend_labels,
        ["Pot center initial", "Pot center current", "Pot center goal"],
    )
    if has_child
        append!(legend_elements, Any[start3_marker, current3_marker])
        append!(legend_labels, ["Child Start", "Child Current"])
    ...
    gripper_distances = [
        sqrt((x1[k] - x2[k])^2 + (y1[k] - y2[k])^2 + (z1[k] - z2[k])^2) for
...
function plot_single_integrator_3d_trajectories(; kwargs...)
    CairoMakie.activate!()
    _plot_single_integrator_3d_trajectories(CairoMakie; kwargs..., interactive = false)
end
...
function plot_trajectory_3d_interactive(;
    ...
    figure, ax, time_index, time_horizon =
        _plot_single_integrator_3d_browser_trajectories(
            WGLMakie;
            kwargs...,
            figure_size,
            legend_label_fontsize = 16,
