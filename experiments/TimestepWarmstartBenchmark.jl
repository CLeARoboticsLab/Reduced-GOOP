...
function run_benchmark(;
	...
	num_mpc_steps >= 2 ||
		throw(ArgumentError("Need at least 2 MPC steps to separate cold from warm solves."))
...
function _run_cell(grid, mode, num_mpc_steps, demo_options)
	cell = (; grid_label = grid.label, Δt = grid.Δt, planning_horizon = grid.planning_horizon, mode)
	wall_time = 0.0
	output = try
...
function _report(summaries, verdict; num_mpc_steps)
	...
	for s in summaries
		if s.failed
			println(io, "| $(_grid_name(s)) | $(s.mode) | — | FAILED: $(s.error) | | | | | | |")
			continue
		end
		println(
			io,
	...
	for v in verdict.per_mode
		...
		trace(values, fmt) =
			join((@sprintf("Δt=%g: %s", d, fmt(x)) for (d, x) in zip(v.Δts, values)), ", ")
		println(
			io,
...
function _csv(summaries)
	...
	for s in summaries
		if s.failed
			println(io, "$(s.grid_label),$(s.Δt),$(s.planning_horizon),$(s.mode),true", ","^27)
			continue
		end
		println(
			io,
