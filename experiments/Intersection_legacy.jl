	...
	prioritized_preferences = [
		[
			...
			function (z, θ)
				...
				# 	-goal_deviation .+ 0.01
				# ]
				sum(goal_deviation .^ 2) #+ 0.001sum(sum(u .^ 2) for u in us)
			end, 
...
function demo(;
	...
	max_inner_iters_schedule = fill(100000, length(epsilon_schedule))
	...
	function default_intersection_initial_states(dynamics_model::Symbol)
		if dynamics_model === :planar_double_integrator
			return [-6.0, -1.0, 2.5, 0.0], [1.0, -6.0, 0.0, 1.0]
	...
	run_id = "0_IP_$(goop_version)_four_levels_eps0p1_penalty"
	...
	@info "Using GOOP version: $(goop_version) with solver: $(typeof(solver))"
...
function build_default_warmstart(
	...
	player1_vx_profile = vcat(initial_state1[3], fill(1.5, planning_horizon - 1))
	player1_vy_profile = fill(0.0, planning_horizon)
	player1_warmstart = build_planar_di_velocity_profile_warmstart(
		planning_horizon,
		dynamics,
		initial_state1;
		vx_profile = player1_vx_profile,
		vy_profile = player1_vy_profile,
	)

	player2_vx_profile = fill(0.0, planning_horizon)
	player2_vy_profile = fill(initial_state2[4], planning_horizon)
