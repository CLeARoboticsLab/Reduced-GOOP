	...
	while outer_iters < max_outer_iters || iszero(total_iters)
		...
		while inner_iters < max_inner_iters && (kkt_error > tol) # (!is_fraction_to_boundary_linesearch || kkt_error > tol)
			...
			if linesearch == :fraction_to_boundary
				...
				Jmat = Matrix(∇F)
				Jsvd = svd(Jmat)
				Jsvd_diagnostic = Jsvd
				condition_number = record_condition_number ? _solver_diagnostic_condition_number(Jsvd_diagnostic.S) : NaN
				...
				if record_solver_diagnostics
					α = min(α_σ, α_γ)
					mul!(Jδz, ∇F, δz)
					predicted_residual_norm = norm(F .+ α .* Jδz, 2)
					z_trial .= z
					z_trial[x_dims] .+= α_σ .* δx
					z_trial[mcp.preference_slack_dims] .+= α_σ .* δs
					z_trial[mcp.interior_point_slack_dims] .+= α_σ .* δσ
					z_trial[mcp.inequality_constraint_dual_dims] .+= α_γ .* δγ
					mcp.F!(F_trial, z_trial; θ, ϵ, η = 0.0)
					F_z_next = norm(F_trial, 2)
					_record_solver_diagnostic!(
						solver_diagnostics,
				...
				Jsvd_diagnostic = use_marquardt_scaling && (record_condition_number || record_solver_diagnostics) ? svd(Jmat) : Jsvd
				condition_number = record_condition_number ? _solver_diagnostic_condition_number(Jsvd_diagnostic.S) : NaN
				verbose && record_condition_number && println("condition number of ∇F: ", condition_number)
				...
				while true
					...
					if α >= min_stepsize
						mul!(Jδz, ∇F, δz)
						predicted_residual_norm = norm(F .+ α .* Jδz, 2)
						pred_reduction = F_z^2 - predicted_residual_norm^2
				...
				elseif α ≥ 0.99 # ρ_low ≤ ρ ≤ ρ_high
					...
					printstyled(" 1. (||F|| - ||F + α∇Fδz||) / ||F|| = $((F_z - predicted_residual_norm) / F_z)\n", color = :green) # small if < 1e-2
					printstyled(" ...max(|Jmat * δz|) = $(maximum(abs.(Jmat * δz)))\n", color = :green)
					if ((F_z - predicted_residual_norm) / F_z) < 1e-2
					...
					τ = 1e-8 * maximum(Jsvd_diagnostic.S)
					r = count(Jsvd_diagnostic.S .> τ)
					Uᵣ = Jsvd_diagnostic.U[:, 1:r]
					Fᵣ = Uᵣ * (Uᵣ' * F)
					F_perp = F - Fᵣ
					printstyled(" 2. ||F_perp|| / ||F|| = $(norm(F_perp, 2) / F_z)\n", color = :green) # large is > 0.3
					
					# Compute newton step using Fᵣ
					if kkt_error < 1e-1 && (norm(F_perp, 2) / F_z) > 0.3
						verbose && printstyled("F⟂ is large relative to F. Computing step using Fᵣ\n", color = :red)
						# TODO 
					end
				end
...
function _record_solver_diagnostic!(
	diagnostics,
	limit,
	inner_iter,
	kkt_error,
	...
	singular_values = Jsvd.S
	σmax = maximum(singular_values)
	σmin = minimum(singular_values)
	condition_number = _solver_diagnostic_condition_number(singular_values)
	step_linearized_norm_ratio = kkt_error > 0 ? norm(α .* Jδz, 2) / kkt_error : NaN
	predicted_relative_reduction = kkt_error > 0 ? (kkt_error - predicted_residual_norm) / kkt_error : NaN
	actual_relative_reduction = kkt_error > 0 ? (kkt_error - actual_residual_norm) / kkt_error : NaN
	lm_delta_pred = 0.5 * kkt_error^2 - 0.5 * predicted_residual_norm^2
	lm_delta_actual = 0.5 * kkt_error^2 - 0.5 * actual_residual_norm^2
	lm_gain_ratio = lm_delta_pred > 0 ? lm_delta_actual / lm_delta_pred : -Inf
	range_rtol = 1e-8
	range_rank = count(singular_values .> range_rtol * σmax)
	Uᵣ = Jsvd.U[:, 1:range_rank]
	F_range = Uᵣ * (Uᵣ' * F)
	F_perp = F - F_range
	F_range_norm = norm(F_range, 2)
	F_perp_norm = norm(F_perp, 2)
	coeffs = abs2.(Jsvd.U' * F)
	residual_energy_svd_basis = sum(coeffs)
	residual_energy_frac_sigma_lt_1e_6 = residual_energy_svd_basis > 0 ? sum(coeffs[singular_values .< 1e-6]) / residual_energy_svd_basis : NaN
	residual_energy_frac_sigma_lt_1e_4 = residual_energy_svd_basis > 0 ? sum(coeffs[singular_values .< 1e-4]) / residual_energy_svd_basis : NaN
	residual_energy_frac_sigma_lt_1e_2 = residual_energy_svd_basis > 0 ? sum(coeffs[singular_values .< 1e-2]) / residual_energy_svd_basis : NaN
	residual_energy_frac_sigma_ge_1e_2 = residual_energy_svd_basis > 0 ? sum(coeffs[singular_values .>= 1e-2]) / residual_energy_svd_basis : NaN
	weighted_inverse_sensitivity = residual_energy_svd_basis > 0 ? sum(coeffs ./ max.(singular_values .^ 2, eps())) / residual_energy_svd_basis : NaN
	cumulative_residual_energy = residual_energy_svd_basis > 0 ? cumsum(coeffs) ./ residual_energy_svd_basis : fill(NaN, length(coeffs))
	energy_idx_50 = something(findfirst(>=(0.50), cumulative_residual_energy), 0)
	energy_idx_90 = something(findfirst(>=(0.90), cumulative_residual_energy), 0)
	energy_idx_99 = something(findfirst(>=(0.99), cumulative_residual_energy), 0)
	...
	push!(
		...
		(;
			...
			eta = η,
			kkt_error,
			norm_delta_z = norm(δz, 2),
			step_linearized_norm_ratio,
			predicted_residual_norm,
			predicted_relative_reduction,
			actual_residual_norm,
			actual_relative_reduction,
			lm_delta_pred,
			lm_delta_actual,
			lm_gain_ratio,
			...
			max_singular_value = σmax,
			min_singular_value = σmin,
			condition_number,
			eta_over_sigma_max_sq = σmax > 0 ? η / σmax^2 : Inf,
			eta_over_sigma_min_sq = σmin > 0 ? η / σmin^2 : Inf,
			range_rank,
			F_range_norm_ratio = kkt_error > 0 ? F_range_norm / kkt_error : NaN,
			F_perp_norm_ratio = kkt_error > 0 ? F_perp_norm / kkt_error : NaN,
			residual_energy_svd_basis,
			residual_energy_frac_sigma_lt_1e_6,
			residual_energy_frac_sigma_lt_1e_4,
			residual_energy_frac_sigma_lt_1e_2,
			residual_energy_frac_sigma_ge_1e_2,
			weighted_inverse_sensitivity,
			residual_energy_sigma_at_50 = energy_idx_50 > 0 ? singular_values[energy_idx_50] : NaN,
			residual_energy_sigma_at_90 = energy_idx_90 > 0 ? singular_values[energy_idx_90] : NaN,
			residual_energy_sigma_at_99 = energy_idx_99 > 0 ? singular_values[energy_idx_99] : NaN,
...
function _solver_diagnostic_condition_number(singular_values)
	σmax = maximum(singular_values)
	σmin = minimum(singular_values)
