...
Base.@kwdef struct InteriorPointOptions
	...
	stagnation_eta_boost_enabled::Bool = false
	stagnation_eta_boost_iters::Int = 15
	stagnation_eta_boost_kkt_threshold::Float64 = 1.5
	stagnation_eta_boost_factor::Float64 = 100.0
	stagnation_eta_boost_min::Float64 = 1e-10
...
function solve(
	...
	stagnation_eta_boost_enabled = options.stagnation_eta_boost_enabled
	stagnation_eta_boost_iters = options.stagnation_eta_boost_iters
	stagnation_eta_boost_kkt_threshold = options.stagnation_eta_boost_kkt_threshold
	stagnation_eta_boost_factor = options.stagnation_eta_boost_factor
	stagnation_eta_boost_min = options.stagnation_eta_boost_min
	...
	while outer_iters < max_outer_iters || iszero(total_iters)
		...
		while inner_iters < max_inner_iters && (kkt_error > tol) # (!is_fraction_to_boundary_linesearch || kkt_error > tol)
			...
			if linesearch == :fraction_to_boundary
				...
				Jsvd = @timeit TO "Newton step / linear solve" begin
					if use_marquardt_scaling
						d = vec(sum(abs2, Jdense; dims = 1)) # diagonal entries of Jmat' * Jmat
						d .= max.(d, 1e-16) # prevent Jscaled from having NaN or Inf entries
						Jscaled = Jdense ./ (sqrt.(d))' # J̃ = JD^(-1/2), J is (m, n), d is (n, 1)
						_robust_svd(Jscaled)
					else
				...
				while true
					@timeit TO "Newton step / linear solve" begin
						...
						if use_marquardt_scaling
							scaled_step = -Jsvd.V * (svd_filters .* (Jsvd.U' * residual))
							δz .= scaled_step ./ sqrt.(d) # undo the scaling
						else
			...
			# Stagnation-triggered η boost: when the KKT error has been flat near
			# the solution for a while, the iterate is typically grinding against
			# a (near-)rank-deficient Jacobian with collapsed regularization. The
			# floor makes the boost effective even after η has collapsed to
			# values where a multiplicative bump alone would be a no-op.
			if stagnation_eta_boost_enabled && kkt_error > tol &&
			   kkt_error < stagnation_eta_boost_kkt_threshold &&
			   iters_since_improvement >= stagnation_eta_boost_iters

				@timeit TO "regularization" begin
					η = min(max(η * stagnation_eta_boost_factor, stagnation_eta_boost_min), η_max)
					iters_since_improvement = 0
					verbose && printstyled("Stagnation detected: boosting η to $η.\n"; color = :magenta)
				end
			end
