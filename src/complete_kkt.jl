...
function solve_complete_goop_pdip(
    ...
    for outer in 1:max_outer
        ...
        for inner in 1:max_inner
            ...
            # Armijo backtracking: enforce  s + α·Δs > 0  and  b(y+α·Δy) > 0
            α = 1.0
            for _ in 1:60
                y_new = y_curr .+ α .* Δy
                s_new = s_curr .+ α .* Δs
                if all(s_new .> 0) && all(b_num(y_new) .> 0)
                    if norm(K_rho([y_new; s_new], rho)) ≤ (1.0 - c1*α) * res
                        break
                    end
                end
                α *= beta_ls
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3.  Example — Non-quadratic objectives + coupled nonlinear constraints
# ═══════════════════════════════════════════════════════════════════════════════
#
#  2-player, 2-level GOOP.  z = [x₁; y₁; x₂; y₂]
#  Player 1 controls (x₁,y₁),  player 2 controls (x₂,y₂).
#
#  ── Objectives ──────────────────────────────────────────────────────────────
#    J¹₁ = (x₁-1)⁴                                  [quartic]
#    J¹₂ = (x₁-0.5)²                                [quadratic secondary]
#    J²₁ = (x₂+1)⁴                                  [quartic]
#    J²₂ = (x₂+0.5)²                                [quadratic secondary]
#
#  ── Coupled equality constraints  h^i(z) = 0  ───────────────────────────────
#    h¹: x₁² + y₁² + 0.2·x₂·y₂ = 1                [nonlinear + cross-player]
#    h²: x₂² + y₂² + 0.2·x₁·y₁ = 1                [symmetric]
#
#  ── Coupled inequality constraints  g^i(z) ≥ 0  ─────────────────────────────
#    g¹: 4 − x₁² − 0.5·x₂² ≥ 0                     [elliptic, coupled]
#    g²: 4 − x₂² − 0.5·x₁² ≥ 0                     [symmetric]
# ══════════════════════════════════════════════════════════════════════════════
...
let
    ...
    y0       = fill(0.0, n_tot)
    y0[1:n_z] .= [-1.5, 0.0, 0.0, 1.5, 0.0, 0.0]
    off = n_z
    for i in 1:N
    ...
    @printf("  h¹(z*) = % .2e  (should be 0; h¹ = y₁)\n",   h_fns[1](z_sol)[1])
    @printf("  h²(z*) = % .2e  (should be 0; h² = y₂)\n",   h_fns[2](z_sol)[1])
    @printf("  g¹(z*) = %.6f  (> 0 required; g¹ = 4-x₁²)\n", g_fns[1](z_sol)[1])
    @printf("  g²(z*) = %.6f  (> 0 required; g² = 4-x₂²)\n", g_fns[2](z_sol)[1])
    ...
    if !isempty(res_hist)
        plt = plot(res_hist;
                   ...
                   grid       = true,
                   framestyle = :box)
        savefig(plt, "goop_complete_convergence.png")
    end
end