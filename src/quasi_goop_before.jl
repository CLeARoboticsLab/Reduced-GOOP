...
function QuasiGOOP_to_PDSyst(;
    ...
    println(
        "Make a PrimalDualSysEqn object from callable functions + implement approximations",
    )
    ...
    function set_up_level(priority_level, player_idx)
        ...
        if priority_level < last(ordered_priority_levels)
            ...
            println(
                "objective_$(priority_level) for player $(player_idx) = ",
                objective_ii,
            )
            ...
            auxillary_constraints =
                prioritized_constraints_ii(x, θ) .+ preference_slacks_ii

            if priority_level == first(ordered_priority_levels)
                push!(
                    Lagrangian_terms,
                    Lagrangian_term(equality_constraints[player_idx](x, θ), nothing, 0), #f
                )
                push!(
                    Lagrangian_terms,
                    Lagrangian_term(
            ...
            x_temp = let
                Symbolics.scalarize(only(Symbolics.@variables(z̃[1:total_dimension])))
            end
        ...
        if priority_level == first(ordered_priority_levels)
            ...
            dims = let
                ns = [
                    length(e.expr) for
                    e in F_ii[player_idx].stationarity[priority_level - 1]
                ]
        ...
        prev_equality_dimension_ii =
            sum(length(e.expr) for e in F_ii[player_idx].equalities[priority_level])
    ...
    final_equality = reduce(
        vcat,
        (t.expr for player in 1:num_players for t in F_ii[player].equalities[end]),
    )
    # Don't forget shared equality constraints (TODO: shared inequality constraints)
