...
struct ParametricGameClassifier{T1,T2,T3,T4,T5,T6,T7,T8,T9}
    ...
    "Callable objectives"
    callable_objectives::T9
end

function ParametricGameClassifier(;
    objectives,
    ...
    # Store objectives
    slacks_as_objectives = fill(Symbolics.Num(0), num_players)
    ...
    function set_up_classifier(priority_level, player_idx)
        ...
        # Define objective as slacks
        slacks_as_objectives[player_idx] += sum(slacks_ii)
    ...
    for player in 1:num_players
        ...
        for priority_level in ordered_priority_levels
            set_up_classifier(priority_level, player)
        end
    end
...
function total_dim(problem::ParametricGameClassifier)
...
"Classify a given game."
function classify_game(
