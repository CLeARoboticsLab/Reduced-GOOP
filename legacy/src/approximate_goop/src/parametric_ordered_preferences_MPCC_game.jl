...
function solve_relaxed_pop_game(
    ...
    while complementarity_residual > tolerance && ii ≤ max_iterations + 1
        ...
        if verbose
            ...
            T = 42
            solution_primals =
                [solution.primals[i][1:T] for i in 1:length(problem.objectives)]
            ...
            println(
                "Check collision w/ each other (1-2): ",
                sqrt.(
                    (solution_primals[1][1:6:end] .- solution_primals[2][1:6:end]) .^ 2 +
                    (solution_primals[1][2:6:end] .- solution_primals[2][2:6:end]) .^ 2,
                ),
            )
            println(
                "Check collision w/ each other (1-3): ",
                sqrt.(
                    (solution_primals[1][1:6:end] .- solution_primals[3][1:6:end]) .^ 2 +
                    (solution_primals[1][2:6:end] .- solution_primals[3][2:6:end]) .^ 2,
                ),
            )
            println(
                "Check collision w/ each other (2-3): ",
                sqrt.(
                    (solution_primals[2][1:6:end] .- solution_primals[3][1:6:end]) .^ 2 +
