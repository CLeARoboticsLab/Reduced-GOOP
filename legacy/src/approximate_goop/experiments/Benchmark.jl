...
function compare_two(one, two; name_one = "First", name_two = "Second")
    ...
    out_folder = joinpath(
        relaxably_feasible_folder,
        string("benchmarks_", Dates.format(now(), "yyyymmdd_HHMMSS")),
    )
    ...
    @showprogress desc = "Benchmarking ..." for file_one in readdir(
        joinpath(folder_one, "solutions");
        join = true,
    )
        ...
        push!(
            ...
            [
                sqrt(
                    sum(
                        (
                            contents_one["strategy1"].xs[k][1:2] -
                            contents_one["strategy2"].xs[k][1:2]
                        ) .^ 2,
                    ),
                ) for k in 1:planning_horizon
            ],
        )
        push!(
            ...
            [
                sqrt(
                    sum(
                        (
                            contents_one["strategy1"].xs[k][1:2] -
                            contents_one["strategy3"].xs[k][1:2]
                        ) .^ 2,
                    ),
                ) for k in 1:planning_horizon
            ],
        ...
        push!(
            ...
            [
                sqrt(
                    sum(
                        (
                            contents_two["strategy1"].xs[k][1:2] -
                            contents_two["strategy2"].xs[k][1:2]
                        ) .^ 2,
                    ),
                ) for k in 1:planning_horizon
            ],
            ...
            [
                sqrt(
                    sum(
                        (
                            contents_two["strategy1"].xs[k][1:2] -
                            contents_two["strategy3"].xs[k][1:2]
                        ) .^ 2,
                    ),
                ) for k in 1:planning_horizon
            ],
        ...
        generic_plot(
            [
                Dict(
                    ...
                    :label => string("B/w Agent 1 & Agent 2 - ", name_one),
                    :color => :red,
                    :marker => :circle,
                ),
                Dict(
                    :data => openloop_distance2_one[1],
                    :label => string("B/w Agent 1 & Agent 3 - ", name_one),
                    :color => :green,
                    :marker => :star4,
                ),
                Dict(
                    :data => openloop_distance3_one[1],
                    :label => string("B/w Agent 2 & Agent 3 ", name_one),
                    :color => :blue,
                    :marker => :star5,
                ),
                Dict(
                    :data => openloop_distance1_two[1],
                    :label => string("B/w Agent 1 & Agent 2 - ", name_two),
                    :color => :red,
                    :marker => :circle,
                ),
                Dict(
                    :data => openloop_distance2_two[1],
                    :label => string("B/w Agent 1 & Agent 3 - ", name_two),
                    :color => :green,
                    :marker => :star4,
                ),
                Dict(
                    :data => openloop_distance3_two[1],
                    :label => string("B/w Agent 2 & Agent 3 ", name_two),
                    :color => :blue,
                    :marker => :star5,
                ),
        ...
        ax1 = Axis(
            fig[1, 1],
            title = "Vehicle Positions ($name_one)",
            xlabel = "X Position",
            ylabel = "Y Position",
        )
        ax2 = Axis(
            fig[1, 2],
            title = "Vehicle Positions ($name_two)",
            xlabel = "X Position",
            ylabel = "Y Position",
        )
        limits!(ax1, -1, 1, -1, 1)
        ...
        scatters1 =
            [scatter!(ax1, [0.0], [0.0], color = c, markersize = 10) for c in colors]
        scatters2 =
            [scatter!(ax2, [0.0], [0.0], color = c, markersize = 10) for c in colors]

        circles1 = [
            lines!(
                ax1,
                cos.(range(0, 2π, length = 100)) * safe_distance_radius .+ 0.0,
        ...
        record(
            fig,
            joinpath(out_folder, string(basename(file_one), "_dual_trajectory.gif")),
            1:length(datasets1[1]);
            framerate = 2,
        ) do i
            # Update Dataset 1
...
function generic_plot(data; title, x_label, y_label, save_to)
    ...
    ax4 = CairoMakie.Axis(fig[1, 1]; xlabel = y_label, ylabel = x_label, title = title)
